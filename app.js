// ==================================
// app.js (النسخة النهائية والمصححة)
// ==================================
...
// 2. منطق المودال (Show Options) - تم إصلاح مشكلة عدم ظهور الخيارات هنا
function showOptions(item, skipOptions = false, itemImage = null){
    selectedItem = item;
    // تحديد الخيار الأول كخيار افتراضي
    selectedOption = item.options && item.options.length > 0 ? item.options[0] : null;
    selectedItemImage = itemImage;
    
    document.getElementById('modalTitle').innerText = item.name;
    const optionsContainer = document.getElementById('modalOptions');
    optionsContainer.innerHTML = ''; // تنظيف الخيارات القديمة

    // --- الكود الذي يضمن عرض أزرار الخيارات ---
    if (item.options && item.options.length > 0) {
        item.options.forEach(option => {
            ...
            // تحديد الخيار الافتراضي عند الفتح
...
// 3. منطق واتساب وإرسال الطلب (هذا الجزء هو مجرد placeholder يحتاج لإكمال)
