...
// تأكد أن هذه المعرفات (IDs) موجودة في ملف HTML الخاص بك
...
function renderMenu(sectionName, searchTerm = ''){
    ...
    if(filteredItems.length === 0 && normalizedSearch.length > 0) {
        menuList.innerHTML = `<p style="text-align:center; padding: 20px; color: rgba(255,255,255,0.6);">لا توجد نتائج بحث في قسم "${sectionName}" في فرع ${currentBranch.name}</p>`;
        return;
    }

    if(filteredItems.length === 0 && normalizedSearch.length === 0 && sectionName !== "الكل") {
        menuList.innerHTML = `<p style="text-align:center; padding: 20px; color: rgba(255,255,255,0.6);">لا تتوفر وجبات في قسم "${sectionName}" حالياً في فرع ${currentBranch.name}.</p>`;
        return;
    }
    ...
    filteredItems.forEach(item => {
        ...
        let priceDisplay;
        if (hasDiscount) {
            priceDisplay = `<span class="old-price" style="text-decoration:line-through; color:#aaa; margin-left:8px;">${item.basePrice}</span> <span class="discount-price" style="color:#ff5252;">${discountedPriceForBranch} ريال</span>`;
        } else {
        ...
        card.innerHTML = `
            ...
            ${ isBestSeller ? '<span class="best-seller-badge" style="position:absolute; top:12px; right:12px; background:#E8C065; color:#000; padding:4px 8px; border-radius:6px; font-size:0.8rem; font-weight:bold;">الأكثر مبيعاً 🏆</span>' : '' }
            <div class="img-wrap">
...
function showOptions(item, skipOptions = false, itemImage = null){
    ...
    if(skipOptions || !item.options || (item.options.length <=1 && item.options[0] && item.options[0].name==="")) {
        ...
        item.options.forEach(opt => {
            ...
            b.onclick = () => {
                selectedOption = opt;
                document.querySelectorAll('#modalOptions .opt-btn').forEach(btn => btn.style.backgroundColor = 'transparent');
                b.style.backgroundColor = 'rgba(232,192,101,0.1)';
            };
            modalOptions.appendChild(b);
        });
        // Select first option by default
        if(modalOptions.children.length > 0) modalOptions.children[0].click();
    }
...
function renderCart(){
    ...
    cart.forEach((it, idx) => {
        ...
        const noteHtml = it.note ? `<div class="item-note-display" style="font-size:0.8rem; color:#E8C065;">📝 ملاحظة: ${it.note}</div>` : '';
        row.innerHTML = `
            <div style="flex-grow:1; min-width:60%">
                <div style="font-weight:800">${it.name}${it.selectedOption && it.selectedOption.name !== '' ? ' — '+it.selectedOption.name : ''}</div>
            ...
            <div class="controls" style="display:flex; align-items:center;">
                <button onclick="updateQty(${idx},-1)" style="background:rgba(255,255,255,0.1); color:#fff; border:none; border-radius:4px; width:25px; height:25px; cursor:pointer;">-</button>
                <div style="min-width:26px;text-align:center">${it.qty}</div>
                <button onclick="updateQty(${idx},1)" style="background:rgba(255,255,255,0.1); color:#fff; border:none; border-radius:4px; width:25px; height:25px; cursor:pointer;">+</button>
                <button onclick="removeItem(${idx})" style="margin-left:6px;background:var(--red);color:#fff;padding:6px;border-radius:6px;border:none;cursor:pointer; font-size:0.8rem;">حذف</button>
            </div>
        `;
...
if(sendWhatsapp) sendWhatsapp.addEventListener('click', () => {
    ...
    cart.forEach(it => {
        const price = (it.basePrice || 0) + (it.selectedOption?it.selectedOption.price:0);
        const optionText = it.selectedOption && it.selectedOption.name !== '' ? ` — ${it.selectedOption.name}` : '';
    ...
    if(deliveryType === 'delivery') {
        ...
        if(userLocation){
            ...
            lines.push(`   * رابط الخريطة: https://www.google.com/maps/search/?api=1&query=${userLocation.lat},${userLocation.lng}`);
...
function onSuccess(position){
    ...
    if(locationStatus) {
        locationStatus.innerHTML = `✅ تم تحديد الموقع بنجاح! <br><a href="${mapLink}" target="_blank" style="color: #E8C065; text-decoration: underline; font-weight: bold;">عرض على الخريطة</a>`;
...
function getMyLocation(){
    ...
    if(!navigator.geolocation){
        if(locationStatus) { locationStatus.innerText = '❌ المتصفح لا يدعم تحديد الموقع الجغرافي.'; locationStatus.style.color = '#D32F2F'; }
...
function flyToCart(imgElement){
    ...
    setTimeout(() => {
        // الخصائص الانتقالية التي سألنا عنها: transition و transform
...
function renderFavoritesShortcut(){
    ...
    // ... logic for displaying favorites (simplified for brevity)
    // You'll need to implement the actual card rendering logic here if you want it to work dynamically

    // Simplified insertion logic (adjust as needed for your layout)
    if(sectionsEl.parentNode) sectionsEl.parentNode.insertBefore(favCard, sectionsEl);
    
    favCard.onclick = () => {
        // Logic to switch view to display only favorites (similar to section switching)
        alert('جارٍ عرض قائمة المفضّلات! (يجب عليك إضافة منطق عرض قائمة المفضّلات في renderMenu)');
        window.scrollTo({top:0, behavior:'smooth'});
    };
}
