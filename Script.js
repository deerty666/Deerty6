        ...
        cartDiv.innerHTML += `
            <div class="cart-item">
                ...
                <div class="no-print" style="display:flex; gap:10px; margin-top:8px;">
                    ...
                    <input placeholder="ملاحظة..." value="${item.note}" oninput="cart[${i}].note=this.value" style="font-size:12px; height:30px;">
                </div>
                ${item.note ? `<div class="print-only" style="font-size:12px; margin-top:2px;">- ${item.note}</div>` : ""}
            </div>`;
    });
...
function sendWhatsApp() {
    ...
    text += `-----------------------%0A*التوصيل:* ${document.getElementById("deliveryFee").value} ريال%0A*الإجمالي: ${document.getElementById("totalPrice").innerText} ريال*%0A%0Aشكراً لزيارتكم! ✨`;
    window.open(`https://wa.me/${phone}?text=${text}`, '_blank');
}

function printOrder() {
    if (cart.length === 0) return alert("أضف أصنافاً أولاً");
    window.print();
    invoiceNumber++;
...
function deleteSingleItem(c, i) { if(confirm("حذف هذا الصنف نهائياً؟")) { data[c].items.splice(i,1); saveMenu(); } }
