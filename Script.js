        ...
        cartDiv.innerHTML += `
            <div style="border-bottom: 1px dashed #ccc; padding: 5px 0;">
                ...
                <div style="display:flex; align-items:center; gap:10px; margin-top:5px;">
                    ...
                    <input class="no-print" placeholder="ملاحظة..." value="${item.note || ''}" oninput="cart[${i}].note = this.value; localStorage.setItem('sahayibCart', JSON.stringify(cart));" style="flex:1; height:25px; font-size:11px;">
                </div>
                ${item.note ? `<div style="font-size:11px; color:red;">* ${item.note}</div>` : ""}
            </div>`;
    });
...
function sendWhatsApp() {
    ...
    let text = `*🧾 فاتورة حجز سحايب ديرتي*%0A--------------------------%0A👤 *العميل:* ${document.getElementById('customerName').value}%0A📍 *الموقع:* ${document.getElementById('customerLocation').value}%0A⏰ *الموعد:* ${document.getElementById('customerTime').value.replace('T', ' ')}%0A--------------------------%0A*📦 الطلبات:*%0A`;
    cart.forEach(item => {
        text += `• ${item.name} × ${item.qty} = *${(item.qty * item.price).toFixed(2)}*%0A${item.note ? `   _(ملاحظة: ${item.note})_%0A` : ""}`;
    });
    text += `--------------------------%0A🚚 *توصيل:* ${document.getElementById("deliveryFee").value} ر.س%0A💰 *الإجمالي النهائي: ${document.getElementById("totalPrice").innerText} ريال*`;
