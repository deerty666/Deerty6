                ...
                <div class="items-grid">
                    ${filteredItems.map((item, iIdx) => `
                        <div class="item-card" onclick="addToCart('${item.name}', ${item.price})">
                            ...
                            <div><span class="item-price">${item.price} ر.س</span></div>
                            <div class="no-print">
                                <button class="del-mini" onclick="event.stopPropagation(); deleteSingleItem(${cIdx}, ${iIdx})">🗑️</button>
                            </div>
                        </div>
                    `).join('')}
                </div>
        ...
        cartDiv.innerHTML += `
            <div class="cart-item">
                ...
                <div class="no-print" style="display:flex; gap:10px; margin-top:5px;">
                    <button onclick="updateQty(${i}, 1)" style="width:35px; padding:2px; background:#27ae60;">+</button>
                    <button onclick="updateQty(${i}, -1)" style="width:35px; padding:2px; background:#e67e22;">-</button>
                    <input placeholder="ملاحظة للطلب..." value="${item.note}" oninput="cart[${i}].note=this.value" style="font-size:12px;">
                </div>
                ${item.note ? `<div class="print-only" style="font-size:12px; font-weight:bold;">- ملاحظة: ${item.note}</div>` : ""}
            </div>`;
    });
...
function sendWhatsApp() {
    ...
    cart.forEach(item => {
        ...
        if(item.note) text += `  _ملاحظة: ${item.note}_%0A`;
    });
    text += `-----------------------%0A*التوصيل:* ${document.getElementById("deliveryFee").value} ريال%0A*الإجمالي النهائي: ${document.getElementById("totalPrice").innerText} ريال*%0A%0Aشكراً لتعاملكم معنا! 🙏`;
...
function deleteSingleItem(c, i) { if(confirm("هل أنت متأكد من حذف الصنف؟")) { data[c].items.splice(i,1); saveMenu(); } }
function saveMenu() { localStorage.setItem("sahayibMenu", JSON.stringify(data)); render(); }
function clearCart() { if(confirm("مسح السلة بالكامل؟")) { cart = []; updateCart(); } }
